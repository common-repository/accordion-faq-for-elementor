...
function afaq_6310_get_user_roles()
{
...
function afaq_6310_explode_class_code($class_code)
{
   $className = '';
   if ($class_code) {
      $classList = explode(',', $class_code);
      if ($classList) {
...
function afaq_6310_scRemove($data)
{
   if($data == '') return ''; 
...
function afaq_6310_extract_data($data)
{
   $array_key = "";
   $array_value = "";
   $i = 0;
   ...
   foreach ($data as $key => $value) {
      if ($i >= 1) {
         ...
         if(substr($value, -6) == 'number'){
            $array_value .= (int) sanitize_text_field($value);
         }
         else{
...
function afaq_6310_split_code($ids, $data)
{
   $css = "";
   $data1 = explode("}", $data);
   if ($data1) {
...
function afaq_6310_link_css_js()
{
   wp_enqueue_style('afaq-6310-font-awesome-5-0-13', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.9.0/css/all.min.css');
   wp_enqueue_style('afaq-6310-codemirror-style', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.59.2/codemirror.min.css');
   wp_enqueue_style('afaq-6310-color-style', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-minicolors/2.3.5/jquery.minicolors.min.css');
   wp_enqueue_script('afaq-6310-color-js', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-minicolors/2.3.5/jquery.minicolors.min.js', array('jquery'));
   wp_enqueue_script('afaq-6310-codemirror-js', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.59.2/codemirror.min.js', array('jquery'));
}

function afaq_6310_faq_faq_install()
{
   global $wpdb;
   ...
   $sql1 = "CREATE TABLE IF NOT EXISTS $style_table (
        ...
        templateids text DEFAULT NULL,
        categoryids text DEFAULT NULL,
        PRIMARY KEY  (id)
      ) $charset_collate;";

   $sql2 = "CREATE TABLE IF NOT EXISTS $faq_table (
        id int UNSIGNED NOT NULL AUTO_INCREMENT,
        title text DEFAULT NULL,
        description text DEFAULT NULL,
        icons_name varchar(100) DEFAULT NULL,
   ...
   $sql3 = "CREATE TABLE IF NOT EXISTS $category_table (
         id int UNSIGNED NOT NULL AUTO_INCREMENT,
   ...
   $class_code = ['', 'afaq-6310-8764', 'afaq-6310-4325', 'afaq-6310-7689', 'afaq-6310-1234', 'afaq-6310-2378', 'afaq-6310-6792'];
   $category = $wpdb->query("select id from $category_table LIMIT 1");
   if (!($category)) {
      $name = ['', 'My Account', 'Getting Started', 'Billing & Payments', 'Mobile App', 'Copyright & Legal', 'Developers'];
      $description = [
         '',
      ...
      $icons = ['', 'fas fa-users-cog', 'fas fa-rocket', 'fas fa-file-invoice-dollar', 'fas fa-mobile-alt', 'fas fa-file-contract', 'fas fa-code'];
   ...
   $install_default = faq_6310_get_option( 'faq_6310_install_default');
   if(!$install_default){
      $wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('faq_6310_install_default', 1)");
   } else{
      return;
   }

   $faq = $wpdb->query("select id from $faq_table LIMIT 1");
   if (!$faq && !$install_default) {
      $title = 'Section';
      $description = "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.";
      $icons = ['', 'fas fa-home', 'fab fa-android', 'fas fa-comment', 'fab fa-apple', 'fas fa-users-cog', 'fas fa-rocket', 'fas fa-file-invoice-dollar', 'fas fa-mobile-alt', 'fas fa-file-contract', 'fas fa-code'];
      ...
      for ($i = 1; $i <= 10; $i++) {
         ...
         while ($cc > 0) {
            $num = rand(1, 6);
            if (!$code || ($code && !in_array($class_code[$num], $code))) {
               $code[] = $class_code[$num];
               $cc--;
            }
         }
         ...
         $sql = "INSERT INTO {$faq_table} (title, description, icons_name, link_text, link_url, class_code) VALUES
            ('{$title} {$i}', '{$description}', '{$icons[$i]}', 'Details', 'https://www.wpmart.com/faq-for-elementor-pro', '{$code}')
            ";
         $wpdb->query($sql);
      }
   }
...
function afaq_6310_add_or_edit_template($id, $faq_order, $cat_order, $style_name)
{
   global $wpdb;
   $style_table = $wpdb->prefix . 'afaq_6310_style';
   $faq_table = $wpdb->prefix . 'afaq_6310_faq';
   ...
   wp_enqueue_media();
?>
   <div class="afaq_6310_add_media">
      <h6>Customize FAQ</h6> 
      <div class="afaq_6310_add_media_body" id="afaq_6310_add_or_edit_faq">
         <i class="fas fa-plus-circle afaq_6310_add_media_add_new_icon"></i>
         Add/Edit FAQ
      </div>
   </div>
   <br />
   <div class="afaq_6310_add_media">
      <h6>Rearrange FAQ</h6>
      <div class="afaq_6310_add_media_body" id="afaq_6310_rearrange_faq">
         <i class="fas fa-cogs afaq_6310_add_media_add_new_icon"></i>
      </div>
   </div>
   <br />
   <div class="afaq_6310_add_media">
      <h6>Customize Category <span class="afaq-6310-pro">(Pro) <div class="afaq-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span></h6>
      <div class="afaq_6310_add_media_body" id="afaq_6310_add_or_edit_category">
         <i class="fas fa-plus-circle afaq_6310_add_media_add_new_icon"></i>
         Add/Edit Category
      </div>
   </div>
   <br />
   <div class="afaq_6310_add_media">
      <h6>Rearrange Category <span class="afaq-6310-pro">(Pro) <div class="afaq-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span></h6>
      <div class="afaq_6310_add_media_body" id="afaq_6310_rearrange_category">
         <i class="fas fa-cogs afaq_6310_add_media_add_new_icon"></i>
      </div>
   </div>
   <br />
   <div class="afaq_6310_add_media">
      <h6>How to Use</h6>
      <div class="afaq_6310_add_media_body">
         <a href="https://www.youtube.com/watch?v=92aD7c1ytmU" target="_blank">
   ...
   $faqArray = [];
   $counterList = $wpdb->get_results("SELECT * FROM $faq_table order by title ASC", ARRAY_A);
   if ($counterList) {
      if ($faq_order) {
         foreach ($faq_order as $co) {
            if ($co) {
              $result = $wpdb->get_row("SELECT * FROM $faq_table where id='" . $co . "'", ARRAY_A);
              if($result){
               $faqArray[] = $result;
              }
            }
      ...
      $counterOrderList = $faqArray;

      foreach ($counterList as $allFaq) {
         if ($faq_order && !in_array($allFaq['id'], $faq_order)) {
            $faqArray[] = $allFaq;
         } else if (!$faq_order) {
            $faqArray[] = $allFaq;
         }
   ...
   // Category
   $catArray = [];
   $categoryList = $wpdb->get_results("SELECT * FROM $category_table order by name ASC", ARRAY_A);
   if ($categoryList) {
      if ($cat_order) {
         foreach ($cat_order as $co) {
            if ($co) {
              $result = $wpdb->get_row("SELECT * FROM $category_table where id='" . $co . "'", ARRAY_A);
              if($result){
               $catArray[] = $result;
              }
            }
      ...
      $categoryOrderList = $catArray;

      foreach ($categoryList as $allCat) {
         if ($cat_order && !in_array($allCat['id'], $cat_order)) {
            $catArray[] = $allCat;
         } else if (!$cat_order) {
            $catArray[] = $allCat;
         }
   ...
   ?>
   <!-- Customize FAQ -->
   <div id="afaq_6310_add_or_edit_faq_modal" class="afaq-6310-modal" style="display: none">
      <div class="afaq-6310-modal-content afaq-6310-modal-md">
         <form action="" method="post">
            <?php wp_nonce_field("afaq_6310_nonce_add_edit_faq") ?>
            <input type="hidden" name="add_edit_faq_id" value="<?php echo $id ?>" />
            <div class="afaq-6310-modal-header">
               Customize Counter
               <span class="afaq-6310-close">&times;</span>
            </div>
            <div class="afaq-6310-modal-body-form">
               <table border="0" width="100%" cellpadding="0" cellspacing="0" class="afaq-member-table">
                  ...
                  <?php
                  if ($faqArray) {
                     foreach ($faqArray as $allFaq) {
                        $checked = '';
                        if ($faq_order && in_array($allFaq['id'], $faq_order)) {
                           $checked = ' checked';
                        }
                        echo "<tr class='afaq-row-select' id='{$allFaq['id']}' height='40'>";
                        echo "<td width='20'><input type='checkbox' name='faqid[]' value='{$allFaq['id']}' id='chk-box-{$allFaq['id']}' class='afaq-row-select-checkbox' {$checked} /></td>";
                        echo "<td>".afaq_6310_scRemove($allFaq['title'])."</td>";
                        echo "</tr>";
                        
                     }
                  } else {
                     echo "No FAQ found. For add FAQ, <a href='" . admin_url("admin.php?page=afaq-6310-manage-faq") . "'>Click Here</a>";
                  }
                  ?>
               </table>
            </div>
            <div class="afaq-6310-modal-form-footer">
               ...
               <input type="submit" name="customize-faq-save" class="afaq-6310-btn-primary afaq-6310-pull-right afaq-6310-margin-right-10" value="Save" />
            </div>
         </form>
      ...
      <!-- Customize Category -->
   <div id="afaq_6310_add_or_edit_category_modal" class="afaq-6310-modal" style="display: none">
      <div class="afaq-6310-modal-content afaq-6310-modal-md">
         <form action="" method="post">
            <?php wp_nonce_field("afaq_6310_nonce_add_edit_category") ?>
            <input type="hidden" name="add_edit_category_id" value="<?php echo $id ?>" />
            <div class="afaq-6310-modal-header">
               Customize Category
               <span class="afaq-6310-close">&times;</span>
            </div>
               ...
               <table border="0" width="100%" cellpadding="0" cellspacing="0" class="afaq-member-table">
                  ...
                  if ($catArray) {
                     foreach ($catArray as $allCat) {
                        ...
                        echo "<tr class='afaq-row-select-cat' id='{$allCat['id']}' height='40'>";
                        echo "<td width='20'><input type='checkbox' name='catid[]' value='{$allCat['id']}' id='chk-box-cat-{$allCat['id']}' class='afaq-row-select-checkbox-cat' {$checked} /></td>";
                        echo "<td>".afaq_6310_scRemove($allCat['name'])."</td>";
                        
                        echo "</tr>";
                     }
                  } else {
                     echo "No Category found. For add category, <a href='" . admin_url("admin.php?page=afaq-6310-manage-category") . "'>Click Here</a>";
                  }
                  ?>
               </table>
            </div>
            <div class="afaq-6310-modal-form-footer">
               <button type="button" name="close" class="afaq-6310-btn-danger afaq-6310-pull-right">Close</button>
               <input type="submit" name="customize-category-save" class="afaq-6310-btn-primary afaq-6310-pull-right afaq-6310-margin-right-10" value="Save" />
            </div>
         </form>
         <br class="afaq-6310-clear" />
      </div>
   ...
   <!-- Rearrange FAQ -->
   <div id="afaq_6310_rearrange_faq_modal" class="afaq-6310-modal" style="display: none">
      <div class="afaq-6310-modal-content afaq-6310-modal-sm">
         <form action="" method="post">
            <?php wp_nonce_field("afaq_6310_nonce_rearrange_faq") ?>
            <input type="hidden" name="rearrange_faq_id" value="<?php echo $id ?>" />
            <input type="hidden" name="rearrange_faq_list" id="rearrange_faq_list" value="<?php echo implode(',', $faq_order) ?>" />
            <div class="afaq-6310-modal-header">
               Rearrange FAQ
               <span class="afaq-6310-close">&times;</span>
            </div>
            <div class="afaq-6310-modal-body-form">
               <ul id="afaq-6310-sortable-faq">
                  <?php
                  if ($counterOrderList) {
                     foreach ($counterOrderList as $allFaq) {
                        echo "<li class='ui-state-default afaq-6310-ui-state-default' id='{$allFaq['id']}'>".afaq_6310_scRemove($allFaq['title'])."</li>";
                     }
                  }
                  ?>
               </ul>
            ...
            <div class="afaq-6310-modal-form-footer">
               <button type="button" name="close" class="afaq-6310-btn-danger afaq-6310-pull-right">Close</button>
               <input type="submit" name="rearrange-faq-save" class="afaq-6310-btn-primary afaq-6310-pull-right afaq-6310-margin-right-10" id="afaq-6310-sortable-faq-sub" value="Save" />
            </div>
         </form>
         <br class="afaq-6310-clear" />
      </div>
      ...
      <!-- Rearrange Category -->
   <div id="afaq_6310_rearrange_category_modal" class="afaq-6310-modal" style="display: none">
      <div class="afaq-6310-modal-content afaq-6310-modal-sm">
         <form action="" method="post">
            <?php wp_nonce_field("afaq_6310_nonce_rearrange_category") ?>
            <input type="hidden" name="rearrange_cat_id" value="<?php echo $id ?>" />
            <input type="hidden" name="rearrange_cat_list" id="rearrange_cat_list" value="<?php echo implode(',', $cat_order) ?>" />
            <div class="afaq-6310-modal-header">
               Rearrange Category
               <span class="afaq-6310-close">&times;</span>
            </div>
            <div class="afaq-6310-modal-body-form">
               <ul id="afaq-6310-sortable-cat">
                  <?php
                  if ($categoryOrderList) {
                     foreach ($categoryOrderList as $allCat) {
                        echo "<li class='ui-state-default afaq-6310-ui-state-default' id='{$allCat['id']}'>".afaq_6310_scRemove($allCat['name'])."</li>";
                     }
                  }
                  ?>
               </ul>
            ...
            <div class="afaq-6310-modal-form-footer">
               <button type="button" name="close" class="afaq-6310-btn-danger afaq-6310-pull-right">Close</button>
               <input type="submit" name="rearrange-cat-save" class="afaq-6310-btn-primary afaq-6310-pull-right afaq-6310-margin-right-10" id="afaq-6310-sortable-cat-sub" value="Save" />
            </div>
         </form>
         <br class="afaq-6310-clear" />
      </div>
   ...
   <style>
      .afaq_6310_6310-ui-state-default {
         padding: 8px 10px;
         cursor: move;
         border-radius: 3px;
      }
   ...
   <script>
      jQuery(function() {
         jQuery("#afaq-6310-sortable-faq").sortable();
         jQuery("#afaq-6310-sortable-faq").disableSelection();
         jQuery("#afaq-6310-sortable-cat").sortable();
         jQuery("#afaq-6310-sortable-cat").disableSelection();
      });

      jQuery(document).ready(function() {
         //Add/Edit Counter Start
         jQuery("body").on("click", ".afaq-row-select-checkbox", function(event) {
            event.stopPropagation();
         });
         jQuery("body").on("click", ".afaq-row-select", function() {
         ...
         // Category
         jQuery("body").on("click", ".afaq-row-select-checkbox-cat", function(event) {
            event.stopPropagation();
         });
         jQuery("body").on("click", ".afaq-row-select-cat", function() {
            var id = jQuery(this).attr("id");
            if (jQuery("#chk-box-cat-" + id).prop('checked') == true) {
               jQuery("#chk-box-cat-" + id).prop('checked', false);
               return false;
            } else {
               jQuery("#chk-box-cat-" + id).prop('checked', true);
               return true;
            }
         });
         ...
         jQuery("#afaq-6310-sortable-faq-sub").click(function() {
            var list_sortable = jQuery('#afaq-6310-sortable-faq').sortable('toArray').toString();
            jQuery("#rearrange_faq_list").val(list_sortable);
         });

         // Rearrange Category
         jQuery("#afaq-6310-sortable-cat-sub").click(function() {
            var list_sortable = jQuery('#afaq-6310-sortable-cat').sortable('toArray').toString();
            jQuery("#rearrange_cat_list").val(list_sortable);
         });

         jQuery("body").on("click", "#afaq_6310_add_or_edit_faq", function() {
            jQuery("#afaq_6310_add_or_edit_faq_modal").fadeIn(500);
            jQuery("body").css({
               "overflow": "hidden"
            });
            return false;
         ...
         jQuery("body").on("click", "#afaq_6310_add_or_edit_category", function() {
            jQuery("#afaq_6310_add_or_edit_category_modal").fadeIn(500);
            jQuery("body").css({
               "overflow": "hidden"
            });
            return false;
         ...
         jQuery("body").on("click", "#afaq_6310_rearrange_faq", function() {
            jQuery("#afaq_6310_rearrange_faq_modal").fadeIn(500);
            jQuery("body").css({
               "overflow": "hidden"
            });
            return false;
         ...
         // Rearrange Category
         jQuery("body").on("click", "#afaq_6310_rearrange_category", function() {
            jQuery("#afaq_6310_rearrange_category_modal").fadeIn(500);
            jQuery("body").css({
               "overflow": "hidden"
            });
            return false;
         ...
         jQuery("body").on("click", "#afaq_6310_add_new_media", function() {
            jQuery("#afaq_6310_add_new_media_modal").fadeIn(500);
            jQuery("body").css({
               "overflow": "hidden"
            });
            return false;
         ...
         jQuery("body").on("click", ".afaq-6310-close, .afaq-6310-btn-danger", function() {
            jQuery("#afaq_6310_add_new_media_modal, #afaq_6310_rearrange_faq_modal, #afaq_6310_add_or_edit_faq_modal").fadeOut(500);
            jQuery("body").css({
               "overflow": "initial"
            });
         });
         jQuery("body").on("click", ".afaq-6310-close, .afaq-6310-btn-danger", function() {
            jQuery("#afaq_6310_add_new_media_modal, #afaq_6310_rearrange_category_modal, #afaq_6310_add_or_edit_category_modal").fadeOut(500);
            jQuery("body").css({
               "overflow": "initial"
            });
         });
         jQuery(window).click(function(event) {
            if (event.target == document.getElementById('afaq_6310_rearrange_faq_modal')) {
               jQuery("#afaq_6310_rearrange_faq_modal").fadeOut(500);
               jQuery("body").css({
                  "overflow": "initial"
               });
            }
            if (event.target == document.getElementById('afaq_6310_add_or_edit_faq_modal')) {
               jQuery("#afaq_6310_add_or_edit_faq_modal").fadeOut(500);
               jQuery("body").css({
                  "overflow": "initial"
               });
            }
            if (event.target == document.getElementById('afaq_6310_add_or_edit_category_modal')) {
               jQuery("#afaq_6310_add_or_edit_category_modal").fadeOut(500);
               jQuery("body").css({
                  "overflow": "initial"
               });
            }
            if (event.target == document.getElementById('afaq_6310_rearrange_category_modal')) {
               jQuery("#afaq_6310_rearrange_category_modal").fadeOut(500);
               jQuery("body").css({
                  "overflow": "initial"
               });
            }
            if (event.target == document.getElementById('afaq_6310_add_new_media_modal')) {
               jQuery("#afaq_6310_add_new_media_modal").fadeOut(500);
               jQuery("body").css({
                  "overflow": "initial"
               });
            }
...
function afaq_6310_color_picker_script()
{
?>
   <script>
      jQuery(document).ready(function() {
         jQuery('.afaq_6310_color_picker').each(function() {
...
function afaq_6310_font_picker_script()
{
?>
   <script>
      
      jQuery(document).ready(function() {
         jQuery('#afaq_6310_jquery_button_font, #afaq_6310_jquery_heading_font, #afaq_6310_jquery_designation_font, #afaq_6310_jquery_heading_font_style1, #afaq_6310_jquery_designation_font_style1, #afaq_6310_jquery_heading_font_style, #afaq_6310_jquery_designation_font_style, #afaq-6310-front-end-load, #afaq_6310_jquery_description_font, #afaq_6310_jquery_contact_font, #afaq_6310_jquery_profile_details_font, #afaq_6310_jquery_category_font_family, #afaq_6310_jquery_read_more_font_family, #afaq_6310_jquery_number_font').fontselect();
      });
   </script>
<?php
}
...
function afaq_6310_items_per_row($id, $rows = NULL)
{
?>
         ...
         <form action="" method="post">
            ...
            <div class="afaq-6310-modal-body-form">
               ...
               <table border="0" width="100%" cellpadding="10" cellspacing="0">
                  <tr>
                     <td width="50%"><label class="afaq-6310-form-label" for="icon_name">Item Per Row:</label></td>
                     <td>
                        <select name="item_per_row_data" class="afaq-6310-form-input">
            ...
            <div class="afaq-6310-modal-form-footer">
               <button type="button" name="close" class="afaq-6310-btn-danger afaq-6310-pull-right">Close</button>
               <input type="submit" name="item_per_row_sub" class="afaq-6310-btn-primary afaq-6310-pull-right afaq-6310-margin-right-10" value="Update" />
            </div>
         </form>
         <br class="afaq-6310-clear" />
      </div>
   ...
   <script>
      ...
      jQuery(document).ready(function() {
         jQuery("body").on("click", "#afaq_6310_items_per_row", function() {
...
<?php
   ...
   function afaq_6310_version_status()
   {
      global $wpdb;
      $db_version = get_option('afaq_6310_version_info');
      if (!$db_version) {
         $wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('afaq_6310_version_info', '" . afaq_6310_PLUGIN_CURRENT_VERSION . "')");
      } else {
         $key = get_option('afaq_6310_license_key');
         if ($db_version != afaq_6310_PLUGIN_CURRENT_VERSION && $key) {
            afaq_6310_check_license($key, true);
            $wpdb->query("UPDATE {$wpdb->prefix}options set 
                           option_value='" . afaq_6310_PLUGIN_CURRENT_VERSION . "' 
                           where option_name = 'afaq_6310_version_info'");
         }
      }
   }


   function afaq_6310_check_license($key, $autoUpdate = false)
   {
      global $wpdb;

      $db_key = get_option('afaq_6310_license_key');
      if (!$db_key) {
         $wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}options SET option_name = %s, option_value = %s", 'afaq_6310_license_key', "{$key}"));
         if (!$wpdb->insert_id) {
            $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}options SET option_value = %s where option_name = %s", "{$key}", 'afaq_6310_license_key'));
         }
      } else if ($db_key != $key) {
         $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}options SET option_value = %s where option_name = %s", "{$key}", 'afaq_6310_license_key'));
      }
      ...
      if (!class_exists('ZipArchive')) {
         $api_params = array(
            ...
            'license' => $key,
            'item_name' => urlencode('Accordion FAQ with Category'),
            'url' => home_url(),
            'type' => 'faq'
         );
         ...
         if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if (false === $license_data->success) {
               switch ($license_data->error) {
                  case 'invalid_key':
                     $message = __('<p class="afaq_6310_error-message">Your have enter invalid license key.</p>');
                     break;
                  case 'site_inactive':
                     $message = __('<p class="afaq_6310_error-message">Your license is not active for this URL.</p>');
                     break;
                  default:
                     $message = __('<p class="afaq_6310_error-message">An error occurred, please try again.</p>');
                     break;
               }
               return;
            }
         ...
         if (is_wp_error($unzipfile)) {
            echo '<p class="afaq_6310_error-message">There was an error unzipping the file.</p>';
            return;
         } else {

            if (!$autoUpdate) {
               echo "<p class='afaq_6310_success-message'>Congratulations! Your license activated successfully.</p>";
         ...
         echo '<iframe width="560" height="315" src="https://www.youtube.com/embed/92aD7c1ytmU" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe> <br /><br />';
         return;
      }
      ...
      if (!$res) {
         ...
         if (!$autoUpdate) {
            echo "<p class='afaq_6310_success-message'>Congratulations! Your license activated successfully.</p>";
            echo "<script>
                     var url = window.location.href + '&status=1'
                     window.location=url;
                  </script>";
         }
      }
...
function afaq_6310_fa_icon_list($startTag, $endTag)
{
...
function afaq_6310_load_search_template($cssData, $ids, $adminPanel = 0)
{
   echo "<div class='active-search-template'>";
   $placeholder = (isset($cssData['place_holder_text']) && $cssData['place_holder_text']  !== '') ? $cssData['place_holder_text'] : 'Enter keywords eg: Foods, blog';
         echo "<div class='afaq-6310-search-template afaq-6310-search-template-number-{$ids}'>
                  <div class='afaq-6310-search-template-main'>
                     <input type='text' data-template-id='{$ids}' placeholder='".afaq_6310_scRemove($placeholder)."' class='afaq-6310-search-template-input afaq-6310-faq-filter'>
...
function afaq_6310_load_category_template($cssData, $ids, $category_order)
{
   global $wpdb;
   $category_table = $wpdb->prefix . 'afaq_6310_category';
   $categoryList = [];
   if($category_order){
      foreach($category_order as $co){
         if($co != ""){
            $result = $wpdb->get_row('SELECT * FROM ' . $category_table . " WHERE id={$co}", ARRAY_A);
            if($result){
               $categoryList[] = $result;
   ...
   echo "<div class='active-category-template'>";
   if (isset($cssData['selected_category_template']) && $cssData['selected_category_template'] !== '') {
      $activate_default_category = isset($cssData['activate_default_category']) ? "afaq-6310-category-wrapper-active": '';
      echo "<div class='afaq-6310-category-template-01'>";
      if ($cssData['selected_category_template'] == 'category-template-01') {
         foreach($categoryList as $cat){
            echo "
               <div class='afaq-6310-category-template-01-wrapper {$activate_default_category}' data-category-class-name='{$cat['class_code']}'>
               <div class='afaq-6310-category-template-01-left-icon'><i class='{$cat['icons']}'></i></div>
               <div class='afaq-6310-category-template-01-content-wrapper'>
                  <div class='afaq-6310-category-template-01-title'> ". afaq_6310_scRemove($cat['name']) ."</div>
                  <div class='afaq-6310-category-template-01-description'>". afaq_6310_scRemove($cat['description']) ."</div>
               </div>
               </div>";
            $activate_default_category = '';
         }
         echo "</div>";
      } else if ($cssData['selected_category_template'] == 'category-template-02') {
         echo "     
      ...
      else if ($cssData['selected_category_template'] == 'category-template-03') {
         echo "<div class='afaq-6310-faq-category-table'>
           <div class='afaq-6310-faq-category-col'>
...
function afaq_6310_load_category_template_output($cssData, $ids, $category_order, $special = '')
{
   global $wpdb;
   $category_table = $wpdb->prefix . 'afaq_6310_category';
   $categoryList = [];
   ...
   echo "<div class='active-category-template'>";
   if (isset($cssData['selected_category_template']) && $cssData['selected_category_template'] !== '') {
      $activate_default_category = isset($cssData['activate_default_category']) && $cssData['activate_default_category'] == 1 ? "afaq-6310-category-wrapper-active": '';
      echo "<div class='afaq-6310-category-template-{$ids}'>";
      if ($cssData['selected_category_template'] == 'category-template-{$ids}') {
         foreach($categoryList as $cat){
            echo "
               <div class='afaq-6310-category-template-{$ids}-wrapper afaq-6310-category-template-wrapper{$special} {$activate_default_category}' data-category-class-name='{$cat['class_code']}'>
               <div class='afaq-6310-category-template-{$ids}-left-icon'><i class='{$cat['icons']}'></i></div>
               <div class='afaq-6310-category-template-{$ids}-content-wrapper'>
                  <div class='afaq-6310-category-template-{$ids}-title'> {$cat['name']}</div>
                  <div class='afaq-6310-category-template-{$ids}-description'>{$cat['description']}</div>
               </div>
               </div>";
            $activate_default_category = '';
         }
...
function faq_6310_get_option($name) {
   global $wpdb;
