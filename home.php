...
if (!empty($_POST['duplicate']) && isset($_POST['id']) && is_numeric($_POST['id'])) {  
   $nonce = $_REQUEST['_wpnonce'];
   if (!wp_verify_nonce($nonce, 'afaq_nonce_field_duplicate')) {
     die('You do not have sufficient permissions to access this page.');
   } else {
     $id = (int) $_POST['id'];
     ...
     $dupList = array(
             ...
             $selectedData['css'], 
             $selectedData['templateids'], 
             $selectedData['categoryids']);
           
     $s = $wpdb->query($wpdb->prepare("INSERT INTO {$style_table} (name, style_name, css, templateids, categoryids) VALUES ( %s, %s, %s, %s, %s )", $dupList));    
...
<table class="afaq-6310-table">
   ...
   foreach ($data as $value) {
      ...
      echo '<tr class="afaq-6310-row-select">';
      echo '<td>' . $value['name'] . '</td>';
      echo '<td>' . ucfirst($style_name[0]) . " " . (int) $style_name[1] . '</td>';
      echo '<td><span>Shortcode <input type="text" style="width: 250px;" onclick="this.setSelectionRange(0, this.value.length)" value="[afaq_6310_faq id=&quot;' . $id . '&quot;]"></span>';
      echo '<td>
                                    <a href="' . admin_url("admin.php?page=afaq-6310-faq-{$temp}&styleid=$id") . '"  title="Edit"  class="afaq-6310-btn-success afaq-6310-margin-right-10" style="float:left; margin-right: 5px; margin-left: 5px; height: 17px"><i class="fas fa-edit" aria-hidden="true"></i></a>
                                    <form method="post">
                                    ' . wp_nonce_field("afaq_nonce_field_duplicate") . '
                                           <input type="hidden" name="id" value="' . $id . '">
                                           <button class="afaq-6310-btn-primary" style="float:left;  margin-right: 5px;"  title="Duplicate"  type="submit" value="duplicate" name="duplicate" onclick="return confirm(\'Do you want to duplicate it?\');"><i class="fas fa-clone" aria-hidden="true"></i></button>
                                   </form>
                                    <form method="post">
